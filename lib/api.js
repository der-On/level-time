...
module.exports = function Api (opts) {
  ...
  api.createTimeEntry = function (group, data) {
    data = data || {};
  ...
  api.tracker.running = function (group, filter) {
    if (!filter) {
      filter = group;
      group = null
    }
    ...
    function _filter(entry) {
      return entry.end ? filter(entry) : false;
    }

    return group ?
      api.tracker.filter(group, _filter)
    : api.tracker.filter(_filter);
  };
